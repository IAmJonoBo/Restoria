...
class GFPGANModel(BaseModel):
    ...
    def setup_optimizers(self):
        ...
        optim_params_g = [{"params": normal_params, "lr": train_opt["optim_g"]["lr"]}]  # add normal params first
        ...
        for _, param in self.net_d.named_parameters():
            normal_params.append(param)
        optim_params_d = [{"params": normal_params, "lr": train_opt["optim_d"]["lr"]}]  # add normal params first
    ...
    def optimize_parameters(self, current_iter):
            ...
            if self.use_facial_disc:
                ...
                if self.opt["train"].get("comp_style_weight", 0) > 0:
                    ...
                    def _comp_style(feat, feat_gt, criterion):
                        ...
                        ) * 0.5 + criterion(self._gram_mat(feat[1]), self._gram_mat(feat_gt[1].detach()))
        ...
        if self.use_facial_disc:
            ...
            l_d_left_eye = self.cri_component(real_d_pred, True, is_disc=True) + self.cri_gan(
            ...
            l_d_right_eye = self.cri_component(real_d_pred, True, is_disc=True) + self.cri_gan(
                fake_d_pred, False, is_disc=True
            )
            ...
            l_d_mouth = self.cri_component(real_d_pred, True, is_disc=True) + self.cri_gan(
                fake_d_pred, False, is_disc=True
            )
